...
void seed_lista(struct lista **cabeca){
    //prepopular a lista para ser mais facil distingir as ações visualmente
    struct lista *pont=NULL;


    pont=malloc(sizeof(struct lista));
    pont->index=10;
    pont->ch='G';
    pont->proximo = malloc(sizeof(struct lista));
    pont->proximo->index=22;
    pont->proximo->ch='A';
    pont->proximo->proximo=malloc(sizeof(struct lista));
...
void imprimir_lista(struct lista * cabeca){
    // imprime todos os valores da lista
    for(struct lista * pont = cabeca; pont!=NULL;pont = pont->proximo){
        printf("\n----------------------\n");
        printf("Indice: %d\n", pont->index);
